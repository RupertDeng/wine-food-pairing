...
def acid_bitter_rule(df, food_nonaromas):
    ...
    if food_nonaromas['acid'][1] == 4:
        df = df.loc[(df['bitter scalar'] <= 2)]
    if food_nonaromas['bitter'][1] == 4:
        df = df.loc[(df['acid scalar'] <= 2)]
    return df

def acid_piquant_rule(df, food_nonaromas):
    # Rule 7: acid and piquant do not go well together
    if food_nonaromas['acid'][1] == 4:
        df = df.loc[(df['piquant scalar'] <= 2)]
    if food_nonaromas['piquant'][1] == 4:
        df = df.loc[(df['acid scalar'] <= 2)]
    return df

def nonaroma_ruling(wine_df, food_nonaroma_values, food_weight):
  ...
  for test in list_of_tests:
    df_test = test(df, food_nonaroma_values)
...
def sweet_pairing(df, food_nonaromas):
    ...
    if food_nonaromas['sweet'][1] == 4:
        df['pairing_type'] = np.where(((df['bitter scalar'] == 4) | (df['fat scalar'] == 4) | (df['piquant scalar'] == 4) | (df['salt scalar'] == 4) | (df['acid scalar'] == 4)), 'contrasting', df['pairing_type'])
    return df

def acid_pairing(df, food_nonaromas):
    # Rule 2: acidic food goes well with highly sweet, fat, or salt wine
    if food_nonaromas['acid'][1] == 4:
        df['pairing_type'] = np.where(((df['sweet scalar'] == 4) | (df['fat scalar'] == 4) | (df['salt scalar'] == 4)), 'contrasting', df['pairing_type'])
    return df
...
def salt_pairing(df, food_nonaromas):
    ...
    if food_nonaromas['salt'][1] == 4:
        df['pairing_type'] = np.where(((df['bitter scalar'] == 4) | (df['sweet scalar'] == 4) | (df['piquant scalar'] == 4) | (df['fat scalar'] == 4) | (df['acid scalar'] == 4)), 'contrasting', df['pairing_type'])
    return df
...
def fat_pairing(df, food_nonaromas):
    ...
    if food_nonaromas['fat'][1] == 4:
        df['pairing_type'] = np.where(((df['bitter scalar'] == 4) | (df['sweet scalar'] == 4) | (df['piquant scalar'] == 4) | (df['salt scalar'] == 4) | (df['acid scalar'] == 4)), 'contrasting', df['pairing_type'])
    return df
...
def congruent_or_contrasting(df, food_nonaromas):
    ...
    for m in most_defining_tastes:
        df['pairing_type'] = df.apply(lambda x: congruent_pairing(x['pairing_type'], food_nonaromas[m][1], x[m + ' scalar']), axis=1)
