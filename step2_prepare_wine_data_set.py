...
if __name__ == '__main__':
  ...
  wine_taste_descriptorized = dict()
  wine_trigram_model = import_wine_phraser()
  for taste in core_tastes:
    taste_df = pd.DataFrame({taste: wine_df['Description'].map(str)})
    wine_taste_descriptorized[taste] = dask_compute(taste_df, 256, 16, map_review_to_taste_descriptor, taste, normalize_sentence, wine_trigram_model, taste_mapping_set[taste])
